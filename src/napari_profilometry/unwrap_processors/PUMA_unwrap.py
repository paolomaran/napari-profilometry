...
def mincut(sourcesink, remain):
    ...
    remain = np.vstack(remain).astype(np.float32) if len(remain) > 0 else np.zeros((0, 4), dtype=np.float32)
...
def unwrap_phase_puma(wrapped_phase: np.ndarray, calib: np.ndarray=None):
    ...
    Returns
    -------
    uw_img: ndarray (sy x sx)
        Image of same dimensions as wrapped_phase, containing the PUMA reconstructions
    '''
